...
def _get_app_root() -> Path:
    ...
    if script_dir.name.lower() == "distr":
        return script_dir.parent
    return script_dir


def _prefer_existing_dir(*candidates: Path) -> Path:
...
def _resolve_initial_dir(current_path: str, fallback_dir: Path) -> str:
    if current_path:
        current = Path(current_path)
        if current.exists():
            return str(current if current.is_dir() else current.parent)
    return str(fallback_dir if fallback_dir.exists() else APP_ROOT)
...
INPUT_DIR = _prefer_existing_dir(APP_ROOT / "Input", APP_ROOT / "input")
OUTPUT_DIR = _prefer_existing_dir(APP_ROOT / "output", APP_ROOT / "Output")
...
def _browse_file(var: tk.StringVar, filetypes, fallback_dir: Path):
    path = filedialog.askopenfilename(
        filetypes=filetypes,
        initialdir=_resolve_initial_dir(var.get().strip(), fallback_dir),
    )
    if path:
        var.set(path)
...
class App(tk.Tk):
    ...
    def _browse_csv(self):
        path = filedialog.askopenfilename(
            ...
            initialdir=_resolve_initial_dir(self._csv_var.get().strip(), INPUT_DIR),
        )
        if not path:
            return
    ...
    def _save_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialdir=_resolve_initial_dir("", _ensure_dir(OUTPUT_DIR)),
        )
        if path:
            try:
